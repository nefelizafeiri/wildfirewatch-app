...
<style>
    ...
    .bc-body { font-size: 0.83rem; line-height: 1.55; color: #b0b4bc; margin-bottom: 8px; }
    .bc-act { font-size: 0.8rem; color: #8b8f97; padding: 2px 0 2px 18px; position: relative; }
    ...
    .overall p { color: #94a3b8; font-size: 0.85rem; line-height: 1.5; margin: 0; }
    .overall strong { color: #cbd5e1; }
...
@st.cache_data
def load_data():
    predictions = pd.read_csv('predictions_with_risk.csv')
    with open('clusters.json') as f: clusters = json.load(f)
    with open('briefing.json') as f: briefing = json.load(f)
    with open('rag_context.txt') as f: rag_context = f.read()
    with open('chatbot_context.txt') as f: chatbot_context = f.read()
    return predictions, clusters, briefing, rag_context, chatbot_context

try:
    predictions, clusters, briefing, rag_context, chatbot_context = load_data()
...
forecast_date = predictions['date'].iloc[0] if 'date' in predictions.columns else 'N/A'
...
with st.sidebar:
    ...
    st.divider()
    show_levels = st.multiselect("Risk levels on map", ['VERY_HIGH','HIGH','MODERATE','LOW'], default=['VERY_HIGH','HIGH','MODERATE'])
...
with brief_col:
    ...
    for cb in briefing.get('clusters', []):
        ...
        css = 'bc-vh' if level == 'VERY_HIGH' else 'bc-h'
        icon = '🔴' if level == 'VERY_HIGH' else '🟠'
        acts = ''.join(f'<div class="bc-act">{a}</div>' for a in cb.get('actions', []))
        st.markdown(f'<div class="bc {css}"><div class="bc-head">{icon} Cluster {cb.get("label","?")} — {cb.get("region","?")} ({level})</div><div class="bc-body">{cb.get("summary","")}</div>{acts}</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="bc bc-ok"><div class="bc-head">🟢 Remaining — Normal Conditions</div><div class="bc-body">{n_l+n_m} hexagons at low or moderate risk. Standard monitoring procedures.</div></div>', unsafe_allow_html=True)
